...
# Sample input CSV rows (for reference):
...
RSpec.describe Appydave::Tools::BankReconciliation::Clean::CleanTransactions do
  ...
  describe '.clean_transactions' do
    ...
    # Pipeline sorts by transaction_date ASC, so the input fixture's row order
    # (12/12, 11/12, 04/12) gets reversed in the output (04/12, 11/12, 12/12).
    context 'when oldest transaction (date 04/12/2023, ASC sort)' do
    ...
    context 'when middle transaction (date 11/12/2023)' do
    ...
    context 'when newest transaction (date 12/12/2023)' do
